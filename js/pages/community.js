...
function setupMoreButtons() {
  ...
  moreButtons.forEach((header, index) => {
    const moreButton =
      header.querySelector("button") || document.createElement("button");
    moreButton.textContent = "더보기 +";
    moreButton.addEventListener("click", () => {
      const categoryId = CATEGORIES[index].id;
      window.location.href = `post_collection.html?category=${encodeURIComponent(
...
async function loadPostsForCategory(categoryId) {
  ...
  try {
    ...
    displayErrorMessage(
      categoryId,
      `게시글을 불러오는데 실패했습니다. 오류: ${error.message}`
    );
  }
}
...
function displayPosts(categoryId, posts) {
  ...
  const postContainer = document.querySelector(
    `.post_category:nth-child(${
      CATEGORIES.indexOf(category) + 1
    }) .category_post_contents`
  );
...
function getToken() {
  ...
  console.log("Current token:", token); // 디버깅을 위해 토큰 출력
  return token;
}
