...
document.addEventListener("DOMContentLoaded", function () {
  ...
  $(".send-chat-btn").on("click", function (e) {
    ...
    if (
      ...
      const method =
        "저희 자세차렷은 사무직 직장인을 위한 자세교정 도움 서비스에요! 대표적인 기능은 '알람형 자세 교정 푸시 알림', '현재 나의 자세 보기', '주변 자세 교정 시설보기', 'AI자세코치' 에요. 기능의 사용법을 설명드리자면 '알람형 자세 교정 푸시 알림' 기능은 사용자가 설정한 일정 시간 간격으로 웹 푸시 알림 발송 기능으로 알람 간격은 10분 / 20분 / 30분 / 1시간 간격으로 설정할 수 있고, 알림과 소리 On/Off도 설정할 수 있어요! 세부 설정은 마이페이지의 '푸시 알림 설정'에서 할 수 있어요. '주변 자세 교정 시설 보기' 기능은 사용자 위치 기반으로 사용자 주변의 자세 교정 시설 정보를 한 눈에 찾아볼 수 있는 기능이에요. 화면 상단의 카테고리바에서 찾기를 원하는 카테고리 항목을 선택하면 지도 위에 마커로 표시됩니다. 원활한 사용을 위해 해당 기능 사용 시 위치 확인 권환을 '허용'으로 설정해 주세요!  '현재 나의 자세 보기' 기능은 사용자의 사용자의 자세 데이터를 분석하여 개선 방향을 제시해주는 기능이에요. 사용자 마다 다른 체형에 맞게 거북목을 판단하고, 추후 다른 질환도 추가할 예정이에요! 현재 나의 자세 보기의 실시간 자세 추적 기능이용하면 사용자의 자세 데이터를 얻어와 해당 데이터를 바탕으로 사용자의 자세를 분석해줘요. 커뮤니티 기능도 존재하니 다양한 기능을 활용해보시길 바랄게요";
      return addChat(method, "AI");
    }
    if (msg.includes("안녕")) {
    ...
    if (msg.includes("감사") | msg.includes("고마워")) {
      const message2 = `아닙니다. ${name}님만의 AI자세코치이니 언제든 편하게 질문하세요!`;
      return addChat(message2, "AI");
    }
